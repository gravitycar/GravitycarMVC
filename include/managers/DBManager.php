...
class DBManager extends Singleton
{
     ...
     *
     * @param hash $defs - the property definitios for a property.
    ...
    public function generateSQLInsert($graviton, $data)
    {
        ...
        foreach ($graviton->propdefs as $fieldName => $defs) {
            ...
            $dataFieldName = "{$graviton->name}_{$fieldName}";
    ...
    public function generateSQLUpdate($graviton, $data)
    {
        ...
        $clauses = array();
        $graviton->id = $data["{$graviton->name}_id"];
        foreach ($graviton->propdefs as $fieldName => $defs) {
            if (!$this->propertyIsInDB($defs)) {
                continue;
            }
            ...
            if ($defs['required'] && empty($data[$dataFieldName])) {
                if (!empty($defs['defaultvalue'])) {
                    ...
                    $this->errMgr->error("Cannot update {$graviton->name} - $fieldName is empty and has no default!");
                    return '';
                }
            }
